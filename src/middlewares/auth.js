...
const isAuth = async(req, res, next) => {
    try {
        ...
        //console.log("autorizacion recibida del front: "+ authorization)
        if(!authorization){
        ...
        //console.log("token tras split: "+token)
        if(!token){
        ...
        console.log("verificacion token: "+tokenVerified.id)
        const userLogged = await User.findById(tokenVerified.id);
        //console.log("paso por try de is auth y req.user es: "+req.user+" -- userlogged es: "+userLogged)
        req.user = userLogged;
